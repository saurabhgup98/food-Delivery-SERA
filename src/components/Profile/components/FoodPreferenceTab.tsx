...
import { CuisinePreferencesI, DietaryPreferencesI } from '../ProfilePageInterfaces';
import { ValidationModal } from '../../common';
    ...
    <div className="space-y-8">
      ...
      <div className="flex items-center justify-between">
        ...
        <PrimaryCenteredActionBtn
          onClick={handleSave}
          disabled={isLoading}
          text={isLoading ? 'Saving...' : 'Save Preferences'}
          size="md"
          className="px-4 py-2"
        />
      </div>
          ...
          {Object.entries(dietaryPreferences).map(([key, value]) => (
            <div key={key} className={`flex items-center p-3 rounded-lg border-2 transition-all duration-200 ${value
              ...
              <input
                ...
                onChange={() => handleDietaryChange(key as keyof DietaryPreferencesI)}
      ...
      <div className="bg-dark-700 rounded-lg p-6 border border-dark-600">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {Object.entries(cuisinePreferences).map(([key, value]) => (
            <button
              key={key}
              onClick={() => handleCuisineChange(key as keyof CuisinePreferencesI)}
